...
- **First-run notice.** Feynman now prints a one-time notice on stderr saying telemetry is on, what it collects, and how to turn it off with `FEYNMAN_TELEMETRY=off`. `feynman status` shows whether telemetry is on.
